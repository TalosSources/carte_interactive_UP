...
class RegionTranslation(gis_models.Model):
    sk3_id = models.IntegerField(null=True, blank=True, unique=True)
    title = models.CharField(max_length=127)  # including "alla områden"
    welcome_message = models.CharField(max_length=32767)  # called welcome_message in sk3
    region = models.ForeignKey(Region, on_delete=models.CASCADE, related_name="r_translations")
    language = models.ForeignKey(Language, related_name='r_language', on_delete=models.PROTECT)
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['region', 'language'], name='max one translation per languageKey and region')
        ]
