...
const NavBar = ( { activeRegion }: {activeRegion : Region}) => {
    ...
    useEffect(() => {
        if (regionPageContainer.current && !fold) {
    ...
    useEffect(() => {
        ...
        console.log(`Taking decision: Available space ${availableSpace}. Recorded widths: ${regionPageContainerWidth} => Fold: ${newFold}`)
    }, [regionPageContainerWidth, availableSpace])
    console.log(`Available space ${availableSpace}. Recorded widths: ${regionPageContainerWidth} => Fold: ${fold}`)

    console.log(`Windowsize: ${windowSize.width}. NavBarWidth ${navbarWidth} - Logowidth: ${logoTitleWidth} - LanguageWidth: ${languageSelectorWidth} = AvailableSpace ${availableSpace}`)
        ...
        <NavRight>
            <div ref={regionPageContainer}><NavItems className="nav-links">
                    ...
                    <Menu id="burger-menu" right styles={ styles }>
                        {activeRegion.properties.rp_region.map(rp =>
                                <Link className="menu-item" to={'/r/'+activeRegion.properties.slug+'/'+rp.slug}>
                                    {t('region.'+activeRegion.properties.slug+'.'+rp.slug+'.title')}
                                </Link>)}
